...
Both reports MUST carry the same `run_id` and `target_id` for the Operational Stack baseline. The Hub now fails manifest generation and bundle validation when these identifiers drift.
...
After generating the bundle, run:

```bash
python tools/validate_operational_stack.py --bundle-dir artifacts/operational-stack
```

This validates manifest schema, artifact existence, `sha256` integrity, and identity alignment across CTS, TSPP, manifest, and `metadata/stack-run.json`.
